  ...
  useEffect(() => {
    const fetchConfirmationStatus = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:3333/api/confirmartoken=${token}`);
  ...
  const handleConfirmation = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        const updateResponse = await axios.post('http://localhost:3333/api/atualizar-email-confirmado', { token });
    ...
    <div className='confirmContainer'>
      {!emailConfirmed ? (
        ...
        <div className='confirmContainer'>
          ...
          <AiOutlineCheckCircle className="delete-success-icon" size="200px" color='green' />
